...
interface BasicInfoFormProps {
  ...
  onCajeroVersionChange?: (version: 'web' | 'tpv') => void;
}
...
const BasicInfoForm: React.FC<BasicInfoFormProps> = ({
  ...
  // Estado para almacenar la versión del cajero
  const [cajeroVersion, setCajeroVersion] = useState<'web' | 'tpv' | ''>(
    (formData as any).cajero_version || ''
  );

  // Efecto para sincronizar cajero_version cuando cambia el formData
  useEffect(() => {
    const version = (formData as any).cajero_version;
    if (version === 'web' || version === 'tpv') {
      setCajeroVersion(version);
    }
  }, [(formData as any).cajero_version]);
  ...
  // Handler para el cambio de versión del cajero
  const handleCajeroVersionChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const version = e.target.value as 'web' | 'tpv';
    setCajeroVersion(version);
    
    // Crear evento para actualizar el formData
    const event = {
      target: {
        name: 'cajero_version',
        value: version
    ...
    if (onCajeroVersionChange) {
      onCajeroVersionChange(version);
    }
  };
  ...
  return (
    ...
    >
      ...
      {/* Selector de versión TPV solo visible cuando el rol es 'cajero' */}
      {formData.role === 'cajero' && (
        ...
        >
          <Select
            id="edit-cajero-version"
            label="Versión Cajero"
            name="cajero_version"
            value={cajeroVersion || 'web'}
            onChange={handleCajeroVersionChange}
            options={[
              { value: 'web', label: 'Versión Web' },
              { value: 'tpv', label: 'Versión TPV' }
            ]}
            helpText="Selecciona si el cajero usará la versión web normal o la aplicación TPV flotante"
          />
        </motion.div>
      )}
