...
const useUsers = () => {
  ...
  // Estado para gestión de suscripción
  const [isUnsubscribeModalOpen, setIsUnsubscribeModalOpen] = useState(false);
  const [userToUnsubscribe, setUserToUnsubscribe] = useState<User | null>(null);
  ...
  // Gestionar suscripción de usuario (suscribir/unsubscribe)
  const handleSubscriptionChange = useCallback(async (user: User, subscribe: boolean) => {
    try {
      setIsSubscribing(true);

      if (subscribe) {
        // Suscribir al usuario directamente (sin modal)
        ...
        setFilteredUsers(prevUsers => prevUsers.map(u => u.id === user.id ? updatedUser : u));
        if (selectedUser?.id === user.id) {
          setSelectedUser(updatedUser);
        }
      } else {
        // Mostrar modal de advertencia ANTES de desuscribir
        setUserToUnsubscribe(user);
        setIsUnsubscribeModalOpen(true);
      }
    } catch (error) {
      toast.error(`Error al gestionar suscripción: ${(error as Error).message}`);
    } finally {
      setIsSubscribing(false);
    }
  }, [selectedUser]);

  // Confirmar unsubscribe completo
  const confirmUnsubscribe = useCallback(async (user: User) => {
    try {
      ...
      toast.success('Usuario dado de baja completamente. Ha perdido acceso al sistema de puntos y recompensas.');
      ...
      toast.error(`Error al darse de baja: ${(error as Error).message}`);
    } finally {
  ...
  return {
    ...
    // Estados y funciones de suscripción
