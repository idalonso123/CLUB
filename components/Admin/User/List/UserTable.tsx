...
interface UserTableProps {
  ...
  onSubscriptionChange?: (user: User, subscribe: boolean) => void; // Añadido onSubscriptionChange
...
const UserTable: React.FC<UserTableProps> = ({
  ...
  onSubscriptionChange, // Añadido onSubscriptionChange
  variants
}) => {
  return (
    <motion.div
        ...
        <table className="min-w-full bg-white">
          <thead className="bg-green-50">
            <tr>
              ...
              <th className="py-2 sm:py-3 px-2 sm:px-4 text-center text-xs sm:text-sm font-medium text-green-900 uppercase tracking-wider">Suscrito</th>
            ...
            {users.length > 0 ? (
              users.map((user) => (
                <UserTableRow
                  ...
                  onSubscriptionChange={onSubscriptionChange} // Pasar onSubscriptionChange al componente hijo
                />
              ))
            ) : (
              <tr>
