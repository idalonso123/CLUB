...
const Login: React.FC<LoginProps> = ({ onOpenHelpModal }) => {
  ...
  const [errorType, setErrorType] = useState<'none' | 'email_not_verified' | 'account_disabled' | 'generic'>('none');
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Si el login fue exitoso, el AuthContext se encarga de la redirección
      if (result.success) {
        return;
      }
      
      // Manejar errores sin lanzar excepciones
      if (result.error) {
      ...
      // Este catch solo捕获 errores de red, no errores de autenticación
  ...
  const handleResendVerification = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setErrorMessage(data.message || 'Error al enviar el email de verificación');
      }
    } catch (error) {
  ...
  const isAccountDisabled = errorType === 'account_disabled';
      ...
      {showError && errorMessage && (
        ...
        >
          <div className="flex">
            {isEmailNotVerified ? (
              <>
                <i className="fas fa-envelope mr-2 mt-0.5"></i>
                <div className="flex-1">
                  ...
                  {!resendSuccess ? (
                    ...
                    <p className="mt-2 text-green-600 font-medium">Email enviado. Revisa tu bandeja de entrada.</p>
                  )}
                </div>
              </>
            ) : isAccountDisabled ? (
