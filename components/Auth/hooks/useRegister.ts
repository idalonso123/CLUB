...
// Información de la versión de los términos y condiciones
const TERMS_VERSION = SITE_CONFIG.legalVersions?.termsAndConditions || '1.0.0';
const PRIVACY_POLICY_VERSION = SITE_CONFIG.legalVersions?.privacyPolicy || '1.0.0';
const TERMS_EFFECTIVE_DATE = SITE_CONFIG.legalVersions?.termsEffectiveDate || '2026-04-28';
...
const useRegister = () => {
  ...
  const validate = (): boolean => {
    ...
    } else if (formData.dni.length < 5) {
      newErrors.dni = "El DNI debe tener al menos 5 caracteres";
    }
    if (!formData.email) {
    ...
    if (!formData.password) {
      ...
      newErrors.password = "La contraseña debe contener al menos un carácter especial (ej: !@#$%^&*)";
    }
    ...
    if (isValid) {
      ...
      try {
        ...
        if (formData.photo) {
          ...
          // Datos de aceptación de términos y condiciones (GDPR)
          formDataToSend.append('termsAccepted', String(formData.terms));
          formDataToSend.append('termsVersion', TERMS_VERSION);
          formDataToSend.append('termsAcceptedAt', new Date().toISOString());
          formDataToSend.append('privacyPolicyAccepted', String(formData.privacyPolicy));
          formDataToSend.append('privacyPolicyVersion', PRIVACY_POLICY_VERSION);
          formDataToSend.append('privacyPolicyAcceptedAt', new Date().toISOString());
          // Información del navegador para auditoría
          formDataToSend.append('browserUserAgent', typeof navigator !== 'undefined' ? navigator.userAgent : '');
          formDataToSend.append('browserLanguage', typeof navigator !== 'undefined' ? navigator.language : '');
          formDataToSend.append('browserPlatform', typeof navigator !== 'undefined' ? navigator.platform : '');
          formDataToSend.append('termsEffectiveDate', TERMS_EFFECTIVE_DATE);
          
          console.log("FormData preparado con campos GDPR, enviando petición");
          ...
          const jsonData = {
            ...
            privacyPolicyAccepted: formData.privacyPolicy,
        ...
        if (response.ok) {
          ...
          if (result.user) {
            ...
            if (userId) {
              ...
              // No intentamos auto-login ya que el usuario debe verificar su email primero
              setSuccessMessage(result.message || "¡Registro exitoso! Por favor verifica tu email para activar tu cuenta.");
              setTimeout(() => {
          ...
          if (response.status === 409) {
            ...
            } else if (result.field === "dni") {
              setServerErrors({
                dni: "Este DNI ya está registrado en nuestro sistema",
              });
            } else {
              setServerErrors({
