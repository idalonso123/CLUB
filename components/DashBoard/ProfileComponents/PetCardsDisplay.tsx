...
import PetCardsHistory from './PetCardsHistory';
...
const PetCardsDisplay: React.FC<PetCardsDisplayProps> = ({ itemVariants }) => {
  ...
  const [filterStatus, setFilterStatus] = useState<'all' | 'active' | 'completed' | 'expiring'>('all');
  ...
  // Si hay carnets completados y no es la primera vez que se ve, ocultarlos (se mostrarán en el banner de recompensas)
  const filteredPetCards = React.useMemo(() => {
    if (filterStatus === 'all') {
      // En vista "Todos", ocultar carnets completados (se muestran en rewards)
      return petCards.filter(card => !card.completed);
    }
    if (filterStatus === 'active') return petCards.filter(card => !card.completed && !isCardExpired(card));
    if (filterStatus === 'completed') {
      // En vista "Completados", ocultar carnets completados (se muestran en rewards)
      return [];
    }
  ...
  const completedCount = petCards.filter(card => card.completed).length;
    ...
    return (
      ...
      >
        ...
        <div className="text-center py-8">
          ...
          <p className="text-gray-500">No tienes carnets de mascotas activos.</p>
          <p className="text-sm text-gray-400 mt-2">
            Visita nuestra tienda para obtener tu carnet de mascota y empezar a acumular sellos.
          </p>
        </div>
      </motion.div>
    );
    ...
    >
      ...
      {petCards.length > 0 && (
        <div className="flex mb-4 border border-gray-200 rounded-md overflow-hidden">
          ...
          <button 
            className={`flex-1 py-2 text-xs font-medium ${activeTab === "history" ? "bg-green-100 text-green-700" : "bg-gray-50 text-gray-600 hover:bg-gray-100"}`}
      ...
      {/* Mensaje informativo cuando hay carnets completados */}
      {completedCount > 0 && activeTab === 'active' && (
        <div className="mb-4 p-3 bg-green-50 border border-green-200 rounded-lg">
          <div className="flex items-center text-green-700 text-sm">
            <i className="fas fa-check-circle mr-2"></i>
            <span>
              Tienes {completedCount} carnet{completedCount > 1 ? 's' : ''} completado{completedCount > 1 ? 's' : ''} con saco de pienso gratis. 
              <a href="/rewards" className="underline font-semibold hover:text-green-800 ml-1">
                Ver recompensas
              </a>
            </span>
          </div>
      ...
      {/* Mostrar historial si está seleccionado */}
      {activeTab === 'history' ? (
        <PetCardsHistory itemVariants={itemVariants} />
      ) : (
