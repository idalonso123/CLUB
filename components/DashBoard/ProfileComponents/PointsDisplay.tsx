  ...
  return (
    <>
      ...
      >
          ...
          <div className="mt-2">
            <div className="w-full bg-gray-200 rounded-full h-2.5">
              <motion.div 
                ...
                style={{ width: `${Math.min(safePoints / 10, 100)}%` }}
                initial={{ width: 0 }}
                animate={{ width: `${Math.min(safePoints / 10, 100)}%` }}
                transition={{ duration: 1, ease: "easeOut" }}
              ></motion.div>
            </div>
            <div className="text-xs text-gray-500 mt-1">
              {safePoints >= 500 ? '¡Nivel Girasol alcanzado!' : `${500 - safePoints} puntos más para alcanzar nivel Girasol`}
            </div>
          </div>
        </div>
        ...
        {/* Mensaje de soporte para usuarios que no han recibido sus puntos */}
        <div 
          onClick={openHelpModal}
          className="mt-3 p-2 bg-blue-50 rounded-md border border-blue-100 text-blue-700 text-sm cursor-pointer hover:bg-blue-100 transition-colors"
        >
          <div className="flex items-center">
            <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <span>¿No has recibido tus puntos? Contacta con soporte</span>
          </div>
        </div>
      </motion.div>
