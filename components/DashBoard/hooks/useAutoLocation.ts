...
export function useAutoLocation({ postalCode, country, setCity, setError }: UseAutoLocationProps) {
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (result.success && result.data && result.data.city) {
              ...
              setCity("");
              if (setError) setError("No se encontró la provincia para el código postal proporcionado.");
            }
          } else {
            setCity("");
            if (setError) setError("Error al buscar provincia por código postal.");
          }
        } catch {
          setCity("");
          if (setError) setError("Error al buscar provincia por código postal.");
