...
const useUserProfile = () => {
    ...
    const saveSection = async (section: keyof EditingState | 'photo') => {
        switch (section) {
            ...
            case 'contact':
                ...
                // @ts-ignore: phonePrefix solo existe en tempUserData cuando es contact
                ...
                // Formatear el teléfono con un espacio después del prefijo
                const fullPhone = `${phonePrefix} ${phone}`;
                
                if (await updateProfile({ phone: fullPhone })) {
    ...
    const cancelEdit = (section: keyof EditingState) => {
        ...
        // Limpiar el error de teléfono al cancelar la edición
        if (section === 'contact') {
            setPhoneError(null);
        }
        setIsEditing(prev => ({
    ...
    const startEditing = (section: keyof EditingState) => {
        ...
        if (section === 'contact') {
            ...
            if (foundPrefix) {
                ...
                phone = userData.phone.slice(foundPrefix.length);
