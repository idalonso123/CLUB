  ...
  if (req.method === 'GET') {
    try {
      ...
      const reward = {
        ...
        tipoRecompensa: rewards[0].tipoRecompensa || 'puntos',
        barcodes: barcodes
      };
  ...
  // PUT: Actualizar una recompensa
    ...
    try {
      const { 
        ...
        expiracionActiva,
        duracionMeses,
        cooldownHoras: requestCooldownHoras,
      ...
      if (barcodes && Array.isArray(barcodes)) {
        console.log('=== DEBUG: Actualizando códigos de barras ===');
        console.log('Barcodes recibidos:', JSON.stringify(barcodes));
        ...
        console.log('Barcodes actuales en BD:', JSON.stringify(currentBarcodesResult));
        ...
        for (const barcode of barcodes) {
          console.log('Procesando barcode:', JSON.stringify(barcode));
          
          // Verificar si este barcode existe en la BD por su ID
          const existsInDB = barcode.id && currentBarcodeIds.includes(Number(barcode.id));
          
          if (existsInDB) {
            // Existe en la BD, actualizar
            console.log('Actualizando barcode existente ID:', barcode.id);
            await executeQuery({
              query: 'UPDATE codigos_barras SET codigo = ?, descripcion = ? WHERE id = ?',
              values: [barcode.codigo, barcode.descripcion || '', barcode.id]
            });
          } else {
            // No existe en la BD (es nuevo), insertar
            console.log('Insertando barcode nuevo:', barcode.codigo);
            await executeQuery({
        ...
        const requestedBarcodeIds = barcodes
          .filter(b => b.id && currentBarcodeIds.includes(Number(b.id)))
          .map(b => Number(b.id));
