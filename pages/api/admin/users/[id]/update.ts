...
async function updateUserHandler(req: AuthenticatedRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Incluir cajero_version solo si está definido y el rol es 'cajero'
    let updateQuery = `
      UPDATE personas 
      SET nombres = ?, apellidos = ?, mail = ?, telefono = ?, rol = ?, status = ?
    `;
    let updateValues: any[] = [firstName, lastName, email, phone || null, role, isStatusDefined ? (status ? 1 : 0) : currentStatus];

    if (cajero_version && role === 'cajero') {
      updateQuery += `, cajero_version = ?`;
      updateValues.push(cajero_version);
    }
    ...
    await executeQuery({
      ...
      if (addressExists) {
        ...
        await executeQuery({
          query: `
            UPDATE direcciones
            SET ciudad = ?, provincia = ?, codpostal = ?, pais = ?, direccion = ?
            WHERE codigo = ?
          `,
          values: [city, province, postalCode, country, address === '' ? null : address, id],
        });
      } else {
