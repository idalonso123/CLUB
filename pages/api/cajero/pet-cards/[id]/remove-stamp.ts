...
async function handler(req: AuthenticatedRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (petCard.stamps <= 0) {
      return res.status(400).json({ success: false, message: 'El carnet no tiene sellos para eliminar' });
    }
    if (petCard.completed) {
      return res.status(400).json({ success: false, message: 'No se puede eliminar sellos de un carnet completado' });
    }
    ...
    await executeQuery({
      query: `
        ...
        WHERE id = ?
      `,
      values: [JSON.stringify(stampDates), now, expirationDateFormatted, petCardId]
    });
