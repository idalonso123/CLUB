...
async function handler(req: AuthenticatedRequest, res: NextApiResponse) {
  try {
    ...
    if (req.method === 'POST') {
      const { userId, petName, petType, productName, codigoBarras } = req.body;
      ...
      const result = await executeQuery({
        query: `
          INSERT INTO pet_cards (userId, petName, petType, productName, stamps, completed, createdAt, updatedAt, stamp_dates, expirationDate, isExpired, codigo_barras)
          VALUES (?, ?, ?, ?, 0, 0, ?, ?, ?, ?, 0, ?)
        `,
        values: [userId, petName, petType, productName, now, now, emptyStampDates, expirationDateFormatted, codigoBarras || null]
      });
