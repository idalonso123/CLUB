...
async function handler(
  ...
  if (req.method === 'GET') {
    try {
      ...
      if (trigger_type) {
        query += ' AND a.trigger_type = ?';
        params.push(trigger_type);
      }

      query += ' ORDER BY a.created_at DESC';

      const automations = await executeQuery({ query, values: params });

      return res.status(200).json({ success: true, data: automations as EmailAutomation[] });
    } catch (error: any) {
  ...
  if (req.method === 'POST') {
    try {
      ...
      const query = `
        ...
        (name, trigger_type, trigger_config, template_id, delay_days, delay_hours, conditions, is_active, created_by)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `;
      ...
      const newAutomation = await executeQuery({
        query: `
          SELECT 
            a.*,
            t.name as template_name,
      ...
      return res.status(201).json({
        success: true,
        automation: (newAutomation as any[])[0] as EmailAutomation
      });
    } catch (error: any) {
