...
import { sendRewardRedemptionEmail } from '@/lib/emailService';
    ...
    try {
      ...
      if (reward.canjeo_multiple !== 1) {
        ...
        if (existingRedemptions && existingRedemptions.length > 0) {
          ...
          // Usar ?? en lugar de || para permitir cooldownHoras = 0 (mismo día)
      ...
      // 4. Descontar puntos al usuario usando FIFO (primero los más antiguos)
      // Obtener los registros de puntos ordenados por fecha de caducidad (primero los más próximos a caducar)
      const puntosActivosResult = await executeQuery({
        query: `
          SELECT id, puntos, fecha_caducidad
          FROM puntos_caducidad 
          WHERE persona_id = ? AND caducado = 0 
          ORDER BY fecha_caducidad ASC
        `,
        values: [req.user.userId]
      }) as Array<{ id: number, puntos: number, fecha_caducidad: string }>;
      
      let puntosARestar = reward.puntos;
      let fechaCaducidadDelUltimoLote = null;
      
      // Restar puntos de cada registro hasta completar el total (FIFO)
      for (const registro of puntosActivosResult) {
        if (puntosARestar <= 0) break;
        
        fechaCaducidadDelUltimoLote = registro.fecha_caducidad;
      ...
      // Actualizar el total de puntos en la tabla personas
      await executeQuery({
      ...
      // Enviar email de confirmación de canje (de forma asíncrona, no bloquea la respuesta)
      sendRewardRedemptionEmail(req.user.userId, {
        rewardName: reward.nombre,
        pointsSpent: reward.puntos,
        remainingPoints: userPoints - reward.puntos,
        codigoBarras: codigoBarras || '',
        expirationDate: fechaExpiracion ? fechaExpiracion.toLocaleDateString('es-ES', {
          day: '2-digit',
          month: 'long',
          year: 'numeric'
        }) : undefined,
        redemptionId: canjeId
      }).catch(emailError => {
        // Log del error de email pero no fallar la transacción
        console.error('Error al enviar email de confirmación de canje:', emailError);
      });
