...
export default async function handler(
  ...
  try {
    // Configurar el transporte de correo usando variables de entorno de la cuenta info@
    const transporter = nodemailer.createTransport({
      host: process.env.EMAIL_CONTACT_HOST,
      port: parseInt(process.env.EMAIL_CONTACT_PORT || '465'),
      secure: process.env.EMAIL_CONTACT_SECURE === 'true', // true para 465, false para otros puertos
      auth: {
    ...
    // Enviar el correo al email de contacto (info@)
    await transporter.sendMail({
      from: `ViveVerde <${process.env.EMAIL_CONTACT_FROM}>`,
      to: process.env.EMAIL_CONTACT || process.env.EMAIL_TO,
