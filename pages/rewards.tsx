...
import CarnetRewardsList from "@/components/Rewards/CarnetRewardsList";
import CanjeCarnetModal from "@/components/Rewards/CanjeCarnetModal";
...
const RewardsPage: NextPage = () => {
  ...
  const [recompensasCarnet, setRecompensasCarnet] = useState<RewardCarnetMascota[]>([]);
  const [categories, setCategories] = useState<string[]>([]);
  const [redemptions, setRedemptions] = useState<Redemption[]>([]);
  const [showHistory, setShowHistory] = useState<boolean>(false);
  const [allRewardsCombined, setAllRewardsCombined] = useState<Reward[]>([]);

  const [searchTerm, setSearchTerm] = useState("");
  const [selectedCategory, setSelectedCategory] = useState("");
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [selectedReward, setSelectedReward] = useState<Reward | null>(null);
  const [selectedCarnetReward, setSelectedCarnetReward] = useState<RewardCarnetMascota | null>(null);
  ...
  // Combinar recompensas de carnet con recompensas de puntos cuando cambien
  useEffect(() => {
    const combinarRecompensas = () => {
      // Las recompensas de carnet van primero
      const carnetRewardsAsPoints: Reward[] = recompensasCarnet.map((rc) => ({
        id: rc.id + 100000, // Usar ID + 100000 para distinguir de recompensas normales
        name: rc.productNombre,
        // FIX: Usar description de la API (plantilla) en lugar de hardcodear
        // La API ya devuelve el valor correcto de la plantilla de carnet
        description: rc.description || 'Has completado un carnet de mascota. El próximo saco te lo llevas gratis!! Entrega este mensaje en caja',
        points: 0,
        tipoRecompensa: 'carnet' as const,
        imageUrl: rc.imageUrl || '/images/saco-gratis.png',
        available: !rc.canjeada && !rc.usada && !rc.isExpired,
        // FIX: Usar category de la API (plantilla) en lugar de hardcodear
        // La API ya devuelve el valor correcto de la plantilla de carnet
        category: rc.category || 'Alimentación',
        // FIX: Usar stock de la API (plantilla) en lugar de hardcodear -1
        // La API ya devuelve el valor correcto de la plantilla de carnet
        stock: rc.stock !== undefined ? rc.stock : -1,
        // FIX: Usar canjeoMultiple de la API (plantilla) en lugar de hardcodear false
        // La API ya devuelve el valor correcto de la plantilla de carnet
        canjeoMultiple: rc.canjeoMultiple || false,
        expiracionActiva: !!rc.fechaExpiracion,
        // FIX: Usar duracionMeses de la API (plantilla) en lugar de hardcodear 1
        // La API ya devuelve el valor correcto de la plantilla de carnet (3 meses)
        duracionMeses: rc.duracionMeses || 3,
        // FIX: Usar cooldownHoras de la API (plantilla) en lugar de hardcodear 0
        // La API ya devuelve el valor correcto de la plantilla de carnet
        cooldownHoras: rc.cooldownHoras ?? 0,
        // FIX: Usar cooldownMode de la API (plantilla) en lugar de hardcodear same_day
        // La API ya devuelve el valor correcto de la plantilla de carnet
        // Validar que sea uno de los valores permitidos
        cooldownMode: (rc.cooldownMode === 'same_day' || rc.cooldownMode === '24_hours' || rc.cooldownMode === 'custom' 
          ? rc.cooldownMode 
          : 'same_day') as 'same_day' | '24_hours' | 'custom',
        barcodes: rc.barcodes || [],
        redeemed: false,
        // Campos adicionales para manejo de recompensas de carnet
        isCarnetReward: true,
        carnetRewardId: rc.id,
        productPvp: typeof rc.productPvp === 'string' ? parseFloat(rc.productPvp) || 0 : (rc.productPvp || 0),
        petName: rc.petName,
        petType: rc.petType,
        productBarcode: rc.productBarcode,
        fechaExpiracion: rc.fechaExpiracion,
      }));

      // Combinar: recompensas de carnet primero, luego recompensas de puntos
      setAllRewardsCombined([...carnetRewardsAsPoints, ...rewards]);
    };

    combinarRecompensas();
  }, [rewards, recompensasCarnet]);
  const fetchRewards = async () => {
    try {
      ...
      if (data.success) {
        // Filtrar recompensas de tipo 'carnet' de la lista de puntos (son plantillas)
        const pointRewards = data.rewards.filter((r: any) => r.tipoRecompensa !== 'carnet');
        setRewards(pointRewards);
        setCategories(data.categories || []);
        return pointRewards; // Devolver las recompensas para combinar después
      } else {
  ...
  // Cargar recompensas de carnet de mascota generadas para el usuario
  const fetchRecompensasCarnet = async () => {
    try {
      const response = await fetch("/api/rewards/carnet-user");
      if (!response.ok) {
        throw new Error("Error al cargar recompensas de carnet");
      }

      const data = await response.json();

      if (data.success) {
        // Solo actualizar el estado - la combinación se maneja en el useEffect
        setRecompensasCarnet(data.recompensas);
      } else {
        console.error("Error en la respuesta:", data.message);
      }
    } catch (error) {
  ...
  const handleRedeemClick = (reward: Reward) => {
    // Verificar si es una recompensa de carnet
    if (reward.isCarnetReward && reward.carnetRewardId) {
      // Buscar la recompensa de carnet original
      const carnetReward = recompensasCarnet.find(rc => rc.id === reward.carnetRewardId);
      if (carnetReward) {
        setSelectedCarnetReward(carnetReward);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        {showHistory ? (
          ...
          <>
            ...
            {/* Lista de recompensas - Todas las recompensas combinadas (carnet primero) */}
            <RewardsList
              rewards={allRewardsCombined}
            ...
            {/* Modal de canje para recompensas de carnet */}
            {selectedCarnetReward && (
              <CanjeCarnetModal
                isOpen={isCarnetModalOpen}
                onClose={() => {
                  setIsCarnetModalOpen(false);
                  setSelectedCarnetReward(null);
                }}
                recompensa={selectedCarnetReward}
                onCanjeado={() => {
                  // Recargar las recompensas de carnet después de canjear
                  fetchRecompensasCarnet();
                  setIsCarnetModalOpen(false);
                  setSelectedCarnetReward(null);
                }}
