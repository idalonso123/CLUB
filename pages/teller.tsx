...
const TellerPage: React.FC = () => {
  ...
  // Verificación de rol (admin o cajero)
  useEffect(() => {
    const checkPermission = async () => {
      try {
        ...
        if (
          ...
          setUserRole(profile.user.role); // Guardar el rol
        } else {
  ...
  if (!isAuthorized) {
    return (
      <div className="max-w-xl mx-auto mt-20 text-center">
        <Head>
          <title>Cajero - Club ViveVerde</title>
          <meta
          name="description"
        ...
        <p>Solo administradores o cajeros pueden acceder a esta sección.</p>
      </div>
    );
  }
  ...
  return (
    <>
      ...
      <TellerDashboard userRole={userRole} />
    </>
  );
};
