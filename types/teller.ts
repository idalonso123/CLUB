...
export interface AddBalanceModalProps {
  ...
  sacos: SacoBarrasItem[];
  setSacos: (sacos: SacoBarrasItem[]) => void;
...
export interface ProductoCarnet {
  id: number;
  Articulo: string;
  Nombre: string;
  Talla: string | null;
...
// SacoBarrasItem se define más abajo, después de ProductoCarnet
...
export interface PetCardModalProps {
  ...
  user: any;
  petCards: PetCard[];
  onAddPetCard: (petName: string, petType: string, productName: string, codigoBarras?: string) => void;
  ...
  userRole?: string | null;
  preselectedProduct?: ProductoCarnet | null;
}

export interface SacoBarrasItem {
  id: string;
  codigoBarras: string;
  pvp: number;
  petCardId: number;
...
// Nueva interfaz para recompensas de carnet de mascota generadas para un usuario
export interface RewardCarnetMascota {
  id: number;
  userId: number;
  petCardId: number;
  productArticulo: string;
  ...
  // Campos adicionales
  imageUrl?: string;
  barcodes?: { id?: number; codigo: string; descripcion?: string }[];
  isExpired?: boolean;
  templateId?: number;
  name?: string;
  description?: string;
